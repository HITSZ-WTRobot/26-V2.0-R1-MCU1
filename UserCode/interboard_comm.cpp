...
static uint8_t AppendTlvI32(uint8_t* out_buf, uint8_t offset, uint8_t type, int32_t value)
{
    if (!out_buf)
    {
        return offset;
...
static void SendFrame(uint8_t cmd, const uint8_t* data, uint8_t data_len)
{
    ...
    (void)HAL_UART_Transmit(&huart4, frame, (uint16_t)(3U + payload_len), kTxTimeoutMs);
}

static void SendAck(uint8_t cmd)
{
    SendFrame((uint8_t)(kAckMask | cmd), nullptr, 0U);
}
                        ...
                        int32_t*       value_out)
{
    ...
    while (idx + 2U <= data_len)
    {
        ...
        {
            *value_out = (int32_t)((uint32_t)data[idx] | ((uint32_t)data[idx + 1U] << 8) |
